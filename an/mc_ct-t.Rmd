...
plotAve <- function(pss, subjStr='subj', xStr='trialT', grpStr='resp', 
    ...
    p <- ggplot(pss, aes_string(x=xStr, y=yStr, colour=grpStr, group=grpStr,
        ...
        scale_x_continuous(breaks=c(.33,.66,1,1.33,1.66,2), 
                           labels=c('0.33','0.66','1','1.33','1.66','2')) +
...
plotIndiv <- function(pss, subjStr='subj', xStr='trialT', grpStr='resp', 
    ...
    p <- ggplot(pss, aes_string(x=xStr, y=yStr, colour=grpStr, group=grpStr,
        ...
        geom_point(position=dodge, size=1, alpha=alpha) + geom_line(position=dodge, alpha=alpha) +
        geom_linerange(position=dodge, show.legend=F, alpha=alpha) +
        scale_x_continuous(breaks=c(.33,.66,1,1.33,1.66,2), labels=c('0.3','','','','','2')) +
        #, limits=c(.3,2))+ #  
...
plotInset <- function(pss, subjStr='subj', xStr='trialT', grpStr='resp', 
                    xlab, ylab=yLab, collab=colLab, yStr='mn', seStr='se'){
    pss$yMin <- pss[,yStr] - pss[,seStr]
    pss$yMax <- pss[,yStr] + pss[,seStr]
    p <- ggplot(pss, aes_string(x=xStr, y=yStr, colour=grpStr, group=grpStr,
        ...
        theme_bw() + theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank(),
                           ...
                           legend.position = 'none', plot.background=element_rect(fill='grey'),
...
sumFn <- function(ss, subjStr='subj', xStr='trialT', grpStr='resp'){
...
sumPFn <- function(ss, subjStr='subj', xStr='trialT', grpStr='resp'){
...
# Replacing pnt IDs with something a little more useful:
ds$subj[ds$subj==9] <- 4
ds$subj[ds$subj==11] <- 5
ds$subj[ds$subj==13] <- 6
ds$subj[ds$subj==14] <- 7
ds$subj[ds$subj==19] <- 8
ds$subj[ds$subj==20] <- 9
...
if(out){jpeg('mc_ct-t_resp_indiv.jpg', width=7, height=3.2, units='in', res=600)}
pRespIndiv
if(out){dev.off()}
```
...
```{r}
m <- lmer(stat ~ trialT + (1|subj), data=ds)
summary(m)
anova(m)
summary(lm(stat ~ trialT, data=ds))
bfBase <- lmBF(stat ~ subj, data=ds, whichRandom = 'subj')
bfSz <- lmBF(stat ~ trialT + subj, data=ds, whichRandom='subj')
as.vector(bfSz / bfBase)
```

## dyna dom ~ static delay
```{r}
m <- lmer(dyna ~ trialT + (1|subj), data=ds)
summary(m)
anova(m)
summary(lm(dyna ~ trialT, data=ds))
bfBase <- lmBF(dyna ~ subj, data=ds, whichRandom = 'subj')
bfSz <- lmBF(dyna ~ trialT + subj, data=ds, whichRandom='subj')
as.vector(bfSz / bfBase)
```

## patchy dom ~ patchy delay
```{r}
m <- lmer(patchy ~ trialT + (1|subj), data=ds)
summary(m)
anova(m)
summary(lm(patchy ~ trialT, data=ds))
bfBase <- lmBF(patchy ~ subj, data=ds, whichRandom = 'subj')
bfSz <- lmBF(patchy ~ trialT + subj, data=ds, whichRandom='subj')
as.vector(bfSz / bfBase)
```

## transparent dom ~ patchy delay
```{r}
m <- lmer(transp ~ trialT + (1|subj), data=ds)
summary(m)
anova(m)
summary(lm(transp ~ trialT, data=ds))
bfBase <- lmBF(transp ~ subj, data=ds, whichRandom = 'subj')
bfSz <- lmBF(transp ~ trialT + subj, data=ds, whichRandom='subj')
as.vector(bfSz / bfBase)
```
...
if(out){jpeg('mc_ct-t_sz_indiv.jpg', width=7, height=3.2, units='in', res=600)}
pIndivSz
if(out){dev.off()}
```

The analysis for center size was performed on the first second of the trial only, as the incidence of static dominance dropped for longer durations.
```{r}
ss <- ds[ds$trialT<=1,]
...
pInsetCs <- plotInset(pss=sumSsAve, xlab=xLab, ylab=yLabCS)
pIndivCs <- plotIndiv(pss=sumSs[sumSs$subj!='average',], xlab=xLab, ylab=yLabCS)
ddply(sumSsAve, .(resp), summarize, respMean=round(mean(mn),2), respSD=round(sd(mn),2))
```

```{r, fig.height=4.2, fig.width=7}
if(out){jpeg('mc_ct-t_cs_indiv.jpg', width=7, height=3.2, units='in', res=600)}
pIndivCs
if(out){dev.off()}
```

The analysis for center-surround trials was performed on the first second of the trial only, as the incidence of static dominance dropped for longer durations.
```{r}
ss <- ds[ds$trialT<=1,]
m <- lmer(ring~ trialT + (1|subj), data=ss)
summary(m)
anova(m)
summary(lm(ring~ trialT, data=ss))
bfBase <- lmBF(ring~ subj, data=ss, whichRandom = 'subj')
bfT <- lmBF(ring~ trialT + subj, data=ss, whichRandom='subj')
as.vector(bfT / bfBase)
```

# Combined plot (Fig.5)
```{r, fig.height=2.1, fig.width=7}
if(out){jpeg('mc_ct-t_fig5.jpg', width=7, height=2.1, units='in', res=600)}
vp <- grid::viewport(x=.757, y=.4, width=.17, height=.35)
grid.arrange(pResp+theme(legend.position='none'), pSz, ncol=2, widths=c(2.21/5,2.79/5))
print(pInsetCs, vp=vp)
if(out){dev.off()}
```