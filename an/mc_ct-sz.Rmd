...
```{r}
m <- lmer(slow ~ stimSzDeg + (1|subj), data=ds)
summary(m)
anova(m)
summary(lm(slow ~ stimSzDeg, data=ds))
bfBase <- lmBF(slow ~ subj, data=ds, whichRandom = 'subj')
bfSz <- lmBF(slow ~ stimSzDeg + subj, data=ds, whichRandom='subj')
as.vector(bfSz / bfBase)
```

## Individual data
```{r, fig.height=2.1, fig.width=7}
p <- ggplot(respSubj, aes(x=stimSzDeg, y=pSubjResp, colour=resp, group=resp)) +
    facet_grid(. ~ subj) +
    geom_point(position=dodge, size=1, alpha=alpha) +
    geom_line(position=dodge, alpha=alpha) +
    # geom_linerange(position=dodge, show.legend=F, alpha=alpha) +
    ...
    guides(colour=guide_legend(keyheight=.3, default.unit='inch'))
p <- themefy(p)
if(!out){plot(p)}else{png('mc_ct-sz_resp_indiv.png', width=7, height=2.1, units='in', res=600); plot(p); dev.off()}
```
...
### Individual data
```{r, fig.height=2.1, fig.width=7}
p <- ggplot(sumRingSzPSubj, aes(x=stimSzDeg, y=ringSzPSubj, colour=resp, group=resp)) +
    facet_grid(. ~ subj) +
    geom_point(position=dodge, size=1, alpha=alpha) +
    geom_line(position=dodge, alpha=alpha) +
    labs(x=expression(paste('Stimulus Size (', degree, ')', sep='')),
         y='Center Size (P)', colour='Response\nType') + ylim(0,1) +
    guides(colour=guide_legend(keyheight=.3, default.unit='inch'))
p <- themefy(p)
if(!out){plot(p)}else{png('mc_ct-sz_szP_indiv.png', width=7, height=2.1, units='in', res=600); plot(p); dev.off()}
# if(out){png('mc_ct-sz_szP_indiv_lowres.png', width=7, height=2.1, units='in', res=150); plot(p); dev.off()}
```
...
#sumRingSz$ringSz <- as.factor(sumRingSz$ringSz)
head(sumRingSz)
```
...
p <- ggplot(sumRingP, aes(x=stimSzDeg, y=ringP, colour=resp, group=resp,
    ...
    guides(colour=guide_legend(keyheight=.3, default.unit='inch')) + ylim(0,1)
p <- themefy(p)
...
# Combined plot (Fig.3)
```{r, fig.height=2.1, fig.width=7}
if(out){jpeg('mc_ct-v_fig3.jpg', width=7, height=2.1, units='in', res=600)}
vp <- grid::viewport(x=.78, y=.405, width=.17, height=.35)
