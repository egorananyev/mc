...
p <- ggplot(sumSlow, aes(x=factor(bvMin), y=factor(bvMax), colour=pResp, size=I(1+.1/seResp))) + 
...
p <- themefy(p)
if(!out){plot(p)}else{png('mc_ct-bv_respSlow2d.png', width=3.2, height=1.9, units='in', res=600); plot(p); dev.off()}
```
