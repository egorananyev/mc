...
plotInset <- function(pss, subjStr='subj', xStr=xVar, grpStr='resp', 
                    xlab, ylab=yLab, collab=colLab, yStr='mn', seStr='se'){
    pss$yMin <- pss[,yStr] - pss[,seStr]
    pss$yMax <- pss[,yStr] + pss[,seStr]
    p <- ggplot(pss, aes_string(x=xStr, y=yStr, colour=grpStr, group=grpStr,
...
## stat dom ~ trial duration
```{r}
summary(glmer(stat ~ trialT + (1|subj), data=ds, family='binomial'))
bfBase <- lmBF(stat ~ subj, data=ds, whichRandom = 'subj')
bfSz <- lmBF(stat ~ trialT + subj, data=ds, whichRandom='subj')
as.vector(bfSz / bfBase)
```

## dyna dom ~ trial duration
```{r}
summary(glmer(dyna ~ trialT + (1|subj), data=ds, family='binomial'))
bfBase <- lmBF(dyna ~ subj, data=ds, whichRandom = 'subj')
bfSz <- lmBF(dyna ~ trialT + subj, data=ds, whichRandom='subj')
as.vector(bfSz / bfBase)
```

## patchy dom ~ trial duration
```{r}
summary(glmer(patchy ~ trialT + (1|subj), data=ds, family='binomial'))
bfBase <- lmBF(patchy ~ subj, data=ds, whichRandom = 'subj')
bfSz <- lmBF(patchy ~ trialT + subj, data=ds, whichRandom='subj')
as.vector(bfSz / bfBase)
```

## transparent dom ~ trial duration
```{r}
summary(glmer(transp ~ trialT + (1|subj), data=ds, family='binomial'))
...
if(out){jpeg('f5_t.jpg', width=7, height=2.1, units='in', res=600)}
c3 <- .423
grid.arrange(pResp + theme(legend.position='none') + ggtitle('a'),
             pCs + theme(legend.position='none') + ggtitle('b'),
             pSz + ggtitle('c'), ncol=3, widths=c((1-c3)/2,(1-c3)/2,c3))
if(out){dev.off()}
```