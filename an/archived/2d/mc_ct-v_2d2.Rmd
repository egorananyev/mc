...
p <- ggplot(sumSlow, aes(x=factor(vMin), y=factor(vMax), color=pResp)) + geom_point(size=4) + 
...
p <- themefy(p)
if(!out){plot(p)}else{png('mc_ct-v_slow2d2.png', width=3.2, height=1.9, units='in', res=600); plot(p); dev.off()}
```
...
```{r, fig.height=3, fig.width=5}
p <- ggplot(sumNoDir, aes(x=factor(vMin), y=factor(vMax), size=pResp)) + geom_point() + 
    scale_size(range=c(0,4)) + 
    geom_point(data=sumNoDir, shape=1, aes(x=factor(vMin), y=factor(vMax), size=1),
               show.legend=F, color ='lightgrey') +
    labs(x=expression(paste('Lower Speed (', degree, '/s)', sep='')), 
         y=expression(paste('Higher Speed (', degree, '/s)', sep='')), 
         size='No-Direction\nResponses') 
...
```{r, fig.height=3, fig.width=5}
p <- ggplot(sumRing, aes(x=factor(vMin), y=factor(vMax), color=nResp)) + geom_point(size=4) + 
    #scale_color_gradient(range=c(0,4), limits=c(0,9), breaks=c(0,3,6,9)) + 
...
```{r, fig.height=3, fig.width=5}
p <- ggplot(sumRing, aes(x=factor(vMin), y=factor(vMax), color=pResp)) + geom_point(size=4) + 
