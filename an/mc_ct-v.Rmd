...
# [2018-01-16] Excluding equal velocity trials
ds <- ds[!(ds$vL==ds$vR),]
rownames(ds)=NULL
head(ds)
```
...
## BF analyses
```{r}
ss <- ds[ds$slow==1 & ds$ring==1,]
ss <- ss[!is.na(ss$ring),]
summary(lmer(ringSzP ~ vMin * vMax + (1|subj), data=ss))
summary(lm(ringSzP ~ vMin * vMax, data=ss))
...
vp <- grid::viewport(x=.323, y=.373, width=.12, height=.3)
vp2 <- grid::viewport(x=.83, y=.373, width=.12, height=.3)
grid.arrange(p2dSlow, pCS, ncol=2, widths=c((5*w)/5,(5*(1-w))/5))
print(pSlowIn, vp=vp)
print(pCSIn, vp=vp2)
if(out){dev.off()}
```
